...
RAG answering an NZ AI policy question with source attribution.
...
a `rag-query` trace showing nested retrieval and generation spans with latency and token counts.

### DeepEval scores
<!-- The eval summary output (Faithfulness / Answer Relevancy / Contextual metrics). -->
![DeepEval results](docs/img/03-deepeval-scores.png)
DeepEval metric scores across the golden question set.
...
auto-generated, sourced PDF briefing with a data table and chart.
...
Granite models served locally via Ollama on the DGX Spark.
